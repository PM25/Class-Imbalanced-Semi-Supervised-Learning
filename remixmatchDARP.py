...
from models.remixmatchDARP.remixmatchDARP import ReMixMatchDARP
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            model.model = torch.nn.parallel.DistributedDataParallel(
                model.model, device_ids=[args.gpu], broadcast_buffers=False, find_unused_parameters=True
            )
    ...
    train_dset = CISSL_Dataset(alg=args.alg, name=args.dataset, train=True, num_classes=args.num_classes, data_dir=args.data_dir)
    lb_dset, ulb_dset = train_dset.get_lb_ulb_data(
    ...
    _eval_dset = CISSL_Dataset(alg=args.alg, name=args.dataset, train=False, num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    loader_dict["eval"] = get_data_loader(dset_dict["eval"], args.eval_batch_size, num_workers=args.num_workers, drop_last=False)

    ## set DataLoader
    model.set_data_loader(loader_dict)
    model.set_dset(dset_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ulb_dist_path", type=str, default=None)
    parser.add_argument("--resume", type=str2bool, default=False)
    parser.add_argument("--load_path", type=str, default=None)
    parser.add_argument("--overwrite", type=str2bool, default=False)
    parser.add_argument(
        "--use_tensorboard", action="store_true", help="Use tensorboard to plot and save curves, otherwise save the curves locally."
    )

    """
    Training Configuration
    """
    parser.add_argument("--alg", type=str, default="remixmatch")
    parser.add_argument("--epoch", type=int, default=1)
    parser.add_argument("--num_train_iter", type=int, default=2**20, help="total number of training iterations")
    parser.add_argument("--num_eval_iter", type=int, default=1000, help="evaluation frequency")
    parser.add_argument("--batch_size", type=int, default=64, help="total number of batch size of labeled data")
    parser.add_argument(
        "--eval_batch_size", type=int, default=1024, help="batch size of evaluation data loader (it does not affect the accuracy)"
    )
    ...
    parser.add_argument("--use_da", type=str2bool, default=True, help="Whether to use distribution alignment")
    ...
    DARP Configurations
    """
    parser.add_argument("--darp_warm", type=int, default=100000, help="number of warm up iterations for DARP")
    parser.add_argument("--darp_alpha", type=float, default=2.0, help="hyperparameter for removing noisy entries")
    parser.add_argument("--darp_iter", type=int, default=10, help="total number of optimization iterations for DARP")
    parser.add_argument("--darp_tol", type=float, default=0.1, help="the allowable error of the zero value for DARP optimization")
    parser.add_argument("--darp_update_iter", type=float, default=10, help="apply DARP frequency")
    ...
    parser.add_argument("--dist-url", default="tcp://127.0.0.1:10001", type=str, help="url used to set up distributed training")
